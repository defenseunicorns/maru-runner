...
func TestTaskRunner(t *testing.T) {
	...
	t.Run("test that env vars get used for variables that do not have a default set", func(t *testing.T) {
		t.Parallel()
		err := os.Setenv("MARU_LOG_LEVEL", "debug")
		require.NoError(t, err)
		err = os.Setenv("MARU_TO_BE_OVERWRITTEN", "env-var")
		require.NoError(t, err)
		stdOut, stdErr, runErr := e2e.Maru("run", "echo-env-var", "--file", "src/test/tasks/tasks.yaml")
		err = os.Unsetenv("MARU_LOG_LEVEL")
		require.NoError(t, err)
		err = os.Unsetenv("MARU_TO_BE_OVERWRITTEN")
		require.NoError(t, err)
